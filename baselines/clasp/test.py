...
from baselines.clasp.clasp import extract_clasp_cps_from_multivariate_ts
# from dtw import dtw
...
def run_clasp_multivariate(X, window_size, num_cps, n_states, offset):
...
def exp_on_synthetic(win_size, num_seg, verbose=False):
    ...
    for i in range(100):
        ...
        prediction = run_clasp_multivariate(data, win_size, num_seg, 5)
...
def exp_on_ActRecTut(win_size, num_seg, verbose=False):
    ...
    for dir_name in dir_list:
        ...
        prediction = run_clasp_multivariate(data, win_size, num_seg, n_states)
        prediction = np.array(prediction, dtype=int)
        result = np.vstack([groundtruth, prediction])
...
def exp_on_MoCap(win_size, num_seg, offset, verbose=False):
    ...
    for fname in os.listdir(base_path):
        ...
        prediction = run_clasp_multivariate(data, win_size, num_seg, n_states, offset)
        prediction = np.array(prediction, dtype=int)
        result = np.vstack([groundtruth, prediction])
...
def exp_on_UCR_SEG(win_size, num_seg, offset, verbose=False):
    ...
    for fname in os.listdir(dataset_path):
        ...
        prediction = run_clasp_multivariate(data, window_size, num_seg, len(seg_info), offset)
        prediction = np.array(prediction, dtype=int)
        result = np.vstack([groundtruth, prediction])
...
def exp_on_PAMAP2(win_size, num_seg, offset, verbose=False):
    ...
    for i in range(1, 9):
        ...
        n_states = len(set(groundtruth))
        prediction = run_clasp_multivariate(data[::10], win_size, num_seg, n_states, offset)
        prediction = np.array(prediction, dtype=int)
        result = np.vstack([groundtruth, prediction])
